...
class RollbarNotifier
{
    ...
    public $root = '';
    public $scrub_fields = ['passwd', 'pass', 'password', 'secret', 'confirm_password',
    ...
    private $config_keys = ['access_token', 'base_api_url', 'batch_size', 'batched', 'branch',
    ...
    public function __construct($config)
    {
        ...
        $levels = [E_WARNING, E_NOTICE, E_USER_ERROR, E_USER_WARNING,
    ...
    protected function _report_exception(Exception $exc, $extra_data = null, $payload_data = null)
    {
        ...
        array_walk_recursive($data, [$this, '_sanitize_utf8']);
    ...
    {
        if (in_array(strtolower($key), $this->scrub_fields, true)) {
            return true;
        }
        foreach ($potential_regex_filters as $potential_regex) {
            if (@preg_match($potential_regex, $key)) {
                return true;
            }
        }
    ...
    protected function make_api_call($action, $access_token, $post_data)
    {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['X-Rollbar-Access-Token: '.$access_token]);
